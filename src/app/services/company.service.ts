...
export class CompanyService {
  ...
  public getApi() {
    this.cognitoService.getToken().then(session => {
      ...
      this._http.get("https://apidev.cartesoft.lu/app/company", httpOptions).subscribe( res => {
        resSubject.next(res);
        console.log(res)
      })
    })
  ...
  public putApi(user:any) {
    this.cognitoService.getToken().then(session => {
      ...
      console.log(user)
      this._http.post("https://apidev.cartesoft.lu/app/company", user, httpOptions).subscribe({
        error: error => {
